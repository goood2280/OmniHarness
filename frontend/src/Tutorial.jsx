...
function tabSteps(lang) {
  const ko = [
    ...
    { icon: '❓', title: '질문',   body: '에이전트가 모호한 결정을 만나면 경영지원 리드가 사용자 언어로 풀어서 올립니다. 답변도 리드가 다시 에이전트 언어로 변환해 전달합니다.' },
    ...
    { icon: '📊', title: '보고서',   body: '의미있는 변경이 쌓이면 보고원이 자동으로 요약 보고서를 발행합니다.' },
  ];
  const en = [
    ...
    { icon: '❓', title: 'Questions',    body: 'When an agent hits an ambiguous decision, the mgmt-lead rewrites it in plain language. Your answer gets translated back into a structured instruction for the agent.' },
    ...
    { icon: '📊', title: 'Reports',      body: 'When meaningful changes pile up, the reporter automatically publishes a summary report.' },
  ];
